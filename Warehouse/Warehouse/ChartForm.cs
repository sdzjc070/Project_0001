                ...
                try
                {
                    ...
                    if (id.Equals("") == false)
                    {
                        ...
                        else
                        {
                            ...
                            string sql = "select * from bindata where BinID = " + id + " and DateTime between '" + start  + "' and '" + end + "'  order by DateTime asc";
                            //DataBase db = new DataBase();
                            //db.command.CommandText = sql;
                            //db.command.Connection = db.connection;
                            ...
                            while (rd.Read())
                            {
                                if (rd[variable].ToString().Equals("") == false)
                                {
                                    if (rd["Volume"].ToString().Equals("-1"))
                                        continue;
                                    DateTime d = DateTime.Parse(rd["DateTime"].ToString());
                                    String date_str = d.ToString();
                                    String[] date_list = date_str.Split(' ');
                                    if (date_list.Length == 2)
                                    {
                                        float f = float.Parse(rd[variable].ToString());
