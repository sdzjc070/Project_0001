    ...
    public class PrintClass
    {
        ...
        private void printdocument_printpage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
                ...
                switch (AlignmentSgin)//对齐方式
                {
                    case 0://左对齐
                    {
                        ...
                        if (tn > cwidth)//如果标题的宽度大于表格的宽度
                            ...
                            xoffset = (int)(PrintPageWidth - (PrintPageWidth) + (cwidth - tn) / 2);//使标题局中
                        break;
                    }
                    case 1://局中
                    {
                ...
                case -1://标题的默认状态
                    {
                        if (tn > cwidth)//如果标题的宽度大于表格的宽度
                        {
                            ...
                            xoffset = (int)(x + (cwidth - tn-100) / 2);//标题相对于表格局中
                        break;
                    }
                }
            }
            ...
            if (this.currentpageindex == 0 || this.iseverypageprinttitle)//当前页为首页，并且每页都要打印标题
            {
                ...
                e.Graphics.DrawString("立筒仓盘库数据报表", titlefont, brush, xoffset, y);
               // e.Graphics.DrawString(this.title, titlefont, brush, xoffset, y);//绘制标题
                y += titlesize+10;//获取标题底端位置
                                  //e.Graphics.DrawString(this.title, titlefont, brush, xoffset, y);//绘制标题
                                  //y += titlesize + 1;//获取标题底端位置
                string code = "";
                try
                {
                    DataBase dbLastData = new DataBase();//查询当前数据库中最新的数据
                    string sql = "select Code from [mqttcode]";
                    ...
                    {
                        code = dbLastData.Dr["Code"].ToString();
                ...
                {
                    MessageBox.Show("数据库查询出错:" + ee.ToString());
                }
               



                e.Graphics.DrawString("公司名称："+ code + "   操作员："+this.title+"           日期：" + t, titlefont, brush, xoffset-230, y);
            ...
            headdown = new Point(cwidth+40, headerheight);//设置右下角位置
            ...
            for (int j = 0; j < colcount; j++)//遍历列数据
            {
                ...
                if (colwidth > 0)//如果列的宽度大于0
                {
                    ...
                    //右侧坚线
                    if ((LineLeft == true) && (j != (colcount - 1)))//如果是左边单元格的线
                        drawline(new Point(x, y), new Point(x, y + currentpagerowcount * rowgap + this.headerheight), e.Graphics, 0);//画线
                    if ((LineLeft==true) && (j == (colcount-1)))//如果是最右边的线
                        drawline(new Point(x+40, y), new Point(x+40, y + currentpagerowcount * rowgap + this.headerheight), e.Graphics, 0);//画线
            ...
            //列标题上边的线
            if (LineUP==true)
                drawline(new Point(leftmargin, y), new Point(rightbound+40, y), e.Graphics, 0); //绘制最上面的线 
            ...
            for (int i = startrow; i < endrow; i++) //对行进行循环
            {
                ...
                //单元格上边的线
                if (LineUnit==true)
                    drawline(new Point(leftmargin, y), new Point(rightbound+40, y), e.Graphics, 0);
                if (Boundary == true && i == startrow)//绘制分割线
                    drawline(new Point(leftmargin, y), new Point(rightbound+40, y), e.Graphics, 1);
                y += rowgap;//设置下行的位置
            }
             //表格最下面的边线
            if (LineUP==true)
                drawline(new Point(leftmargin, y), new Point(rightbound+40, y), e.Graphics, 0);//绘制最下面的线
            currentpageindex++;//下一页的页码
            if (this.needprintpageindex)//如果显示页脚
                e.Graphics.DrawString("共 " + pagecount.ToString() + " 页   第 " + this.currentpageindex.ToString() + " 页", this.footerfont, brush, PrintPageWidth - 150, (int)(PrintPageHeight - this.buttommargin / 2.5 - this.footerfont.Height));//绘制页脚信息
